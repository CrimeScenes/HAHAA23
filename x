...
-- List of lower body parts to include for targeting
local LowerBodyParts = {
    "LeftFoot", "LeftLowerLeg", "LeftUpperLeg", "RightLowerLeg", "RightUpperLeg", "RightFoot"
}
...
-- Check if the mouse is on any part of the target's body, including lower parts
local function isMouseOnTarget(targetPlayer)
    ...
    if mouse.Target and mouse.Target:IsDescendantOf(targetPlayer.Character) then
        local partName = mouse.Target.Name
        return table.find(LowerBodyParts, partName) or true  -- Check if the part is in LowerBodyParts
    end
    return false
end
